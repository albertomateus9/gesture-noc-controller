    ...
    <main>
      <section className="hero">
        ...
        <div className="statusPanel">
          ...
          <small>Sem uploads, sem inferência em nuvem e sem identificação biométrica.</small>
        </div>
      </section>
      {children}
    </main>
...
function FresnelPlanner() {
  const [distance, setDistância] = useState(8);
  const [frequency, setFrequência] = useState(5.8);
  ...
  const report = `# Fresnel Vision Relatório\n\nDistância: ${distance} km\nFrequência: ${frequency} GHz\nMidpoint Raio de Fresnel: ${radius.toFixed(2)} m\nObstacle risk: ${risk}%\n`;
  return (
    <Shell>
      <div className="grid">
        <CanvasBox>
          <svg viewBox="0 0 720 360" role="img" aria-label="Sintético line-of-sight map">
        ...
        <section className="panel">
          <label>Distância {distance} km<input type="range" min="1" max="30" value={distance} onChange={(e) => setDistância(Number(e.target.value))} /></label>
          <label>Frequência {frequency} GHz<input type="range" min="2" max="80" step="0.1" value={frequency} onChange={(e) => setFrequência(Number(e.target.value))} /></label>
          <label>Índice de obstáculo {obstacle}<input type="range" min="5" max="95" value={obstacle} onChange={(e) => setObstacle(Number(e.target.value))} /></label>
          <div className="metrics"><Metric label="Raio de Fresnel" value={`${radius.toFixed(1)} m`} /><Metric label="Folga" value={`${clearance}%`} /><Metric label="Risco" value={`${risk}%`} tone={risk > 55 ? 'warn' : 'ok'} /></div>
          <button onClick={() => downloadText('fresnel-report.md', report)}>Exportar Markdown</button>
        </section>
      </div>
    </Shell>
  );
...
function RackLabelOcrLite() {
  ...
  const rows = labels.map((label, index) => ({ slot: index + 1, label, confidence: textDetectorAvailable ? 0.82 : 0.64, source: textDetectorAvailable ? 'TextDetector API' : 'Fallback sintético' }));
  return (
    <Shell>
      <div className="grid">
        <CanvasBox>
        ...
        <section className="panel">
          <p className="note">Este MVP usa the browser TextDetector API when available and a synthetic local fallback otherwise.</p>
          <textarea value={labels.join('\n')} onChange={(e) => setLabels(e.target.value.split('\n').filter(Boolean))} />
          <div className="metrics"><Metric label="Etiquetas detectadas" value={String(labels.length)} /><Metric label="Mode" value={textDetectorAvailable ? 'OCR nativo' : 'Fallback'} /></div>
          <button onClick={() => downloadText('rack-labels.csv', toCsv(rows))}>Exportar CSV</button>
        </section>
      </div>
    </Shell>
  );
    ...
    <Shell>
      <div className="grid">
        ...
        <section className="panel">
          ...
          <div className="metrics"><Metric label="Grupos de cabos" value={String(counts.filter((c) => c.count > 0).length)} /><Metric label="Cabos estimados" value={String(total)} /></div>
          <button onClick={() => downloadText('cable-colors.json', JSON.stringify(counts, null, 2))}>Exportar JSON</button>
        </section>
      </div>
    </Shell>
  );
...
function GestureNocController() {
  const [events, setEvents] = useState<string[]>(['pronto']);
  const [active, setActive] = useState(0);
  const commands = ['Próximo painel', 'Painel anterior', 'Reconhecer alerta', 'Abrir topologia'];
  ...
  return (
    <Shell>
      <div className="grid">
        <CanvasBox><div className="nocBoard">{commands.map((cmd, i) => <div className={i === active ? 'selected' : ''} key={cmd}>{cmd}<small>canal de gesto {i + 1}</small></div>)}</div></CanvasBox>
        <section className="panel">
          <p className="note">Painel sintético de gestos. Um adaptador futuro com landmarks de mãos do MediaPipe pode alimentar a mesma interface de comandos.</p>
...
function ClassroomOccupancyAnon() {
  ...
  const capacity = 32;
  const risk = people / capacity > 0.85 ? 'Alto' : people / capacity > 0.65 ? 'Moderado' : 'Baixo';
    ...
    <Shell>
      <div className="grid">
        ...
        <section className="panel">
          ...
          <div className="metrics"><Metric label="Occupancy" value={`${Math.round((people / capacity) * 100)}%`} /><Metric label="Privacy" value="Masked" tone="ok" /><Metric label="Risco" value={risk} tone={risk === 'Alto' ? 'warn' : 'ok'} /></div>
          <button onClick={() => downloadText('occupancy.csv', toCsv(rows))}>Exportar CSV</button>
        </section>
      </div>
    </Shell>
  );
    ...
    <Shell>
      <div className="grid">
        ...
        <section className="panel">
          ...
          <button onClick={() => downloadText('depth-survey.json', JSON.stringify({ mode: loaded ? 'transformers-hook' : 'heuristic', contrast: scan }, null, 2))}>Exportar JSON</button>
        </section>
      </div>
    </Shell>
  );
    ...
    <Shell>
      <div className="grid">
        ...
        <section className="panel">
          ...
          <div className="metrics"><Metric label="FPS" value={String(result.fps)} /><Metric label="Latência" value={`${result.latency} ms`} /><Metric label="Score" value={String(result.score)} /></div>
          <button onClick={() => downloadText('edge-cv-benchmark.json', JSON.stringify(result, null, 2))}>Exportar JSON</button>
        </section>
      </div>
    </Shell>
  );
  ...
  return (
    <Shell>
      <div className="grid">
        ...
        <section className="panel">
          <div className="metrics"><Metric label="Frame drop" value={`${latest.frameDrop.toFixed(1)}%`} /><Metric label="Latência" value={`${latest.latency.toFixed(0)} ms`} /><Metric label="Anomalies" value={String(anomaly)} tone={anomaly > 0 ? 'warn' : 'ok'} /></div>
          ...
          <button onClick={() => downloadText('vision-telemetry.csv', toCsv(points))}>Exportar CSV</button>
        </section>
      </div>
    </Shell>
  );
